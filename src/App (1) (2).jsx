...
export default function App() {
  ...
  const handleLogout = () => {
      ...
      // Dibuat "fire and forget" (tidak di-await) supaya proses logout di UI
      // tetap berjalan mulus walau request ke GAS lambat atau gagal (mis. offline).
      fetch(GAS_AUTH_URL, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
Kembalikan seluruh data JSON (English, Indonesia, & Mandarin) secara lengkap dan sinkron.`;
  ...
  if (!isAuthenticated) {
      return (
          <div className="min-h-screen flex items-center justify-center bg-slate-100 relative overflow-hidden" style={{ fontFamily: "'Share Tech', sans-serif", backgroundImage: 'linear-gradient(rgba(59, 130, 246, 0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(59, 130, 246, 0.05) 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
            <style>{`
            ...
            <div className={`flex flex-col items-center justify-center w-full max-w-sm px-4 z-10 transition-all duration-500 ${loginState === 'success' ? 'opacity-0 scale-110' : 'opacity-100 scale-100'}`}>
                ...
                <div className="w-full bg-white p-6 rounded-lg border border-slate-200 shadow-sm flex flex-col gap-4 relative z-10">
                    ...
                    <button onClick={handleLogin} disabled={loginState === 'loading' || loginState === 'success'} className="bg-slate-800 text-white p-3 text-base font-bold rounded-lg cursor-pointer shadow-sm transition hover:bg-slate-900 disabled:opacity-50">LOGIN</button>
                </div>
            </div>
          </div>
      );
