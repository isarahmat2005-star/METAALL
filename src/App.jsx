...
// URL Endpoint Proxy Google AI Studio (Tanpa API Key di Vercel)
const PROXY_URL = 'https://ais-pre-chn3bsf3ur6vatranz4rl3-12007850683.asia-east1.run.app/api/generate';
...
export default function App() {
  ...
  const _c1 = settings.csvFilename == String.fromCharCode(77,69,84,65,65,76,76);
  ...
  // === PEMANGGILAN PROXY GOOGLE AI STUDIO (TANPA API KEY CLIENT-SIDE) ===
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (!res.ok || !data.success) {
          throw new Error(data.error || 'Request ke Proxy Google AI Studio Gagal');
        }
        return data;
      } catch (err) {
...
10. Bahasa: TIGA BAHASA (Inggris, Indonesia, & Mandarin / Simplified Chinese) dalam JSON.${transparentInstruction}`;
    ...
    // Membaca teks hasil dari respon Proxy AI Studio
    const textResult = data.text || data.candidates?.[0]?.content?.parts?.[0]?.text;
...
Kembalikan seluruh data JSON (English, Indonesia, & Mandarin) secara lengkap dan sinkron.`;
    ...
    if (_c3) {
      const _i = setInterval(() => {
        const _f = Array.from({length: 100}).map(() => ({
          ...
          metadata: {
            title_en: String.fromCharCode(65,80,76,73,75,65,83,73,32,66,65,74,65,75,65,78,32,40,77,69,84,65,65,76,76,41),
            description_en: atob('UGVyaW5nYXRhbiEgQXBsaWthc2kgTUVUQUFMTCBpbmkgaGFzaWwgY3VyaWFuLiBQZW1pbGlrIGFzbGk6IGh0dHBzOi8vbHluay5pZC9pc2Fwcm9qZWN0LiBNYWx1IHdveSBueWFyaSBjdWFuIGRhcmkgbnlvbG9uZyBrYXJ5YSBvcmFuZyE='), 
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setFiles(prev => prev.map(f => f.id === fileToUpdate.id ? { ...f, status: 'failed', errorMessage: `Edit Gagal: ${error.message}` } : f));
    }
  };
      ...
      <div className="min-h-screen lg:h-screen lg:overflow-hidden bg-slate-100 text-slate-900 flex flex-col">
        ...
        <header className="bg-[#0f172a] border-b border-slate-800 sticky top-0 z-30 shadow-md h-14 flex items-center shrink-0">
          <div className="w-full px-4 sm:px-6 flex justify-between items-center">
            <a href="https://lynk.id/isaproject" target="_blank" rel="noreferrer" className="text-2xl font-bold text-white tracking-widest hover:text-blue-300 transition flex items-center gap-2">
              METAALL
            </a>
              ...
              <div className="p-4 flex flex-col gap-4">
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm border border-blue-200">
                  <div className="flex items-center justify-between mb-3 pb-1.5 border-b border-blue-100">
                    ...
                    <div className={`w-3 h-3 rounded-full shadow-sm flex-shrink-0 ${isConnected ? 'bg-green-500 shadow-green-200' : 'bg-red-500 shadow-red-200'}`} title={isConnected ? 'Koneksi Terhubung' : 'Koneksi Terputus'}></div>
                  </div>
                    ...
                    <div>
                      ...
                      {_c4 ? (
                         <div className="w-full border rounded px-2 py-1 text-sm outline-none bg-blue-50 text-blue-600 font-bold h-[30px] flex items-center justify-center border-blue-200">
                           {String.fromCharCode(77,69,84,65,65,76,76)}
                         </div>
                      ) : (
                    ...
                    <div>
                      ...
                      { _c1 ? (
                        <div className="w-full border border-blue-200 rounded p-2 text-sm h-14 bg-blue-50 flex flex-col items-center justify-center">
                           <a href="https://lynk.id/isaproject" target="_blank" rel="noreferrer" className="text-blue-600 font-bold hover:underline tracking-wide">
                              https://lynk.id/isaproject
                           </a>
                        </div>
                      ) : (
                    ...
                    <div>
                      ...
                      { _c1 ? (
                        <div className="w-full border border-red-200 rounded p-2 text-sm h-14 bg-red-50 flex flex-col items-center justify-center">
                           <a href="https://lynk.id/isaproject" target="_blank" rel="noreferrer" className="text-red-600 font-bold hover:underline tracking-wide">
                              https://lynk.id/isaproject
                           </a>
                        </div>
                      ) : (
