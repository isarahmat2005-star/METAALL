...
// URL Proxy Google AI Studio (Tanpa API Key di Vercel)
const GEMINI_PROXY_URL = "https://ais-pre-chn3bsf3ur6vatranz4rl3-12007850683.asia-east1.run.app/api/generate";
...
export default function App() {
  ...
  // MENGIRIM REQUEST KE SERVER PROXY GOOGLE AI STUDIO (TANPA API KEY DI VERCEL)
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (!res.ok || data.success === false) {
          throw new Error(data.error || 'Terjadi kesalahan pada proxy');
        }
...
10. Bahasa: TIGA BAHASA (Inggris, Indonesia, & Mandarin / Simplified Chinese) dalam JSON.${transparentInstruction}`;
    ...
    const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error("AI tidak memberikan respon dalam 60 detik.")), 60000)
    );

    const data = await Promise.race([fetchWithRetry(payload, 5, signal), timeoutPromise]);
    
    // Mengekstrak teks respon dari server proxy
                  ...
                  {files.map(f => (
                    <div key={f.id} className={`bg-white hover:shadow-md rounded-lg shadow-sm border flex flex-col transition-all duration-300 ${f.status === 'processing' ? 'border-blue-400 ring-2 ring-blue-100' : f.status === 'failed' ? 'border-red-300' : 'border-blue-200'}`}>
                      ...
                      <div className="grid grid-cols-4 gap-2 p-2 bg-blue-50/50 border-b border-blue-100 rounded-t-lg shrink-0">
                        <button onClick={() => setPreviewFile(f)} className="flex flex-row items-center justify-center gap-1.5 py-1.5 rounded border bg-white border-blue-200 text-blue-600 hover:bg-blue-100 transition-colors" title="Preview File">
                          <EyeIcon />
