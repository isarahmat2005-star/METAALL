...
import { 
  ...
  FilePlusIcon, FolderPlusIcon, FileJsonIcon, FileTextIcon 
} from './icons';
import { 
  ...
  apiKey, HF_CONVERTER_URLS 
} from './constants';
import { loadJSZip, isVectorExt, isVideoFile, handleCopy, calculateTargetSize } from './utils';

// --- ICON TAMBAHAN UNTUK UI BARU ---
const ChevronLeftIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>;
...
const UserIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>;
const LogOutIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" y1="12" x2="9" y2="12"></line></svg>;

// --- KONFIGURASI GOOGLE APPS SCRIPT ---
// Ganti URL ini dengan URL Web App dari Google Apps Script Anda nanti
const GAS_AUTH_URL = "URL_GOOGLE_APPS_SCRIPT_ANDA_DISINI";

// --- FUNGSI INDEXEDDB UNTUK PENYIMPANAN LOKAL ---
const DB_NAME = 'MetaallStorage';
const STORE_NAME = 'metaall_files';
...
const saveFilesToDB = async (files) => {
  try {
    ...
    store.clear();
    files.forEach(f => {
        const { url, ...fileDataToSave } = f; 
        store.put(fileDataToSave);
    });
    return new Promise((resolve, reject) => {
...
const loadFilesFromDB = async () => {
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
          const result = request.result.map(f => {
              return { ...f, url: f.file ? URL.createObjectURL(f.file) : null };
          });
          resolve(result);
      };
      request.onerror = () => reject(request.error);
    });
  } catch (err) { console.error("IDB Load Error:", err); return []; }
};
...
export default function App() {
  // --- STATE OTENTIKASI & NOTIFIKASI ---
  const [authStatus, setAuthStatus] = useState('idle'); // 'idle' | 'checking' | 'authenticated'
  const [userEmail, setUserEmail] = useState('');
  const [loginInput, setLoginInput] = useState('');
  const [animState, setAnimState] = useState('idle'); // 'idle' | 'checking' | 'happy' | 'angry'
  ...
  useEffect(() => {
      let storedId = localStorage.getItem('metaall_device_id');
      if (!storedId) {
          storedId = 'dev_' + Math.random().toString(36).substr(2, 9) + Date.now().toString(36);
          localStorage.setItem('metaall_device_id', storedId);
      }
      setDeviceId(storedId);
      
      const sessionEmail = localStorage.getItem('metaall_session');
      if (sessionEmail) {
  ...
  useEffect(() => {
      if (authStatus === 'authenticated' && !isDBLoaded) {
          loadFilesFromDB().then(savedFiles => {
              setFiles(savedFiles);
              setIsDBLoaded(true);
          });
      }
  }, [authStatus, isDBLoaded]);
  ...
  const handleLogin = async () => {
      ...
      try {
          if(GAS_AUTH_URL === "URL_GOOGLE_APPS_SCRIPT_ANDA_DISINI") {
             // Mockup sukses jika URL belum diisi (untuk tes UI)
             setTimeout(() => {
                 setAnimState('happy');
                 showToast("Selamat Datang Kembali", "success");
                 localStorage.setItem('metaall_session', loginInput.trim());
                 setUserEmail(loginInput.trim());
                 setTimeout(() => setAuthStatus('authenticated'), 1200);
             }, 1500);
             return;
          }
          ...
          const res = await fetch(GAS_AUTH_URL, {
              ...
              body: JSON.stringify({ email: loginInput, deviceId: deviceId, action: 'login' })
          });
  ...
  const handleLogout = async () => {
      try {
          if(GAS_AUTH_URL !== "URL_GOOGLE_APPS_SCRIPT_ANDA_DISINI") {
              // Panggil API Logout untuk membebaskan device
              await fetch(GAS_AUTH_URL, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ email: userEmail, deviceId: deviceId, action: 'logout' })
              });
          }
      ...
      setAuthStatus('idle');
      setIsDBLoaded(false);
      setFiles([]); // Kosongkan state (di DB tetap ada, ditarik pas login lagi)
  ...
  }, [files.length, itemsPerPage, currentPage, totalPages]);
  ...
  const fetchWithRetry = (payload, retries = 5, signal) => {
    return new Promise((resolve, reject) => {
      if (window.parent === window) {
        return reject(new Error("Aplikasi harus dijalankan di dalam iframe Gemini Canvas untuk terhubung ke AI."));
      }
      ...
      const messageListener = (event) => {
        ...
        if (data && data.type === 'GEMINI_RESPONSE' && data.id === requestId) {
          ...
          else { reject(new Error(data.error || "Gagal mendapatkan respon dari Canvas")); }
        }
      };
      ...
      window.parent.postMessage({ type: 'CALL_GEMINI', id: requestId, payload: payload }, '*');
    });
  };
...
Kembalikan seluruh data JSON (English, Indonesia, & Mandarin) secara lengkap dan sinkron.`;
  ...
  const handleExportCSV = async () => {
    ...
    if (platform === 'Export All (ZIP)') {
      ...
      try {
        ...
        showToast("Gagal membuat file ZIP. Pastikan koneksi internet aktif.", "error");
      } finally {
        setIsZipping(false);
      }
      return;
  ...
  const getStatusBorderColor = () => {
      ...
      if (countSuccess > 0 && countSuccess === displayTotalFiles && displayTotalFiles > 0) return 'border-green-300';
  ...
  // --- RENDER LOGIN SCREEN (Jika belum authenticated) ---
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-slate-100 relative overflow-hidden" style={{ backgroundImage: 'linear-gradient(rgba(59, 130, 246, 0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(59, 130, 246, 0.05) 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
            <style>{`
                ...
                /* Tema Minimalis (Biru, Merah, Hijau) */
                ...
                .state-checking .theme-color { stroke: #3b82f6; } .state-checking .theme-fill { fill: #3b82f6; } .state-checking .aura-glow { fill: #3b82f6; opacity: 0.2; }
                .state-checking .ring-1 { animation: spin-cw 3s linear infinite; } .state-checking .ring-2 { animation: spin-ccw 2s linear infinite; } .state-checking .ring-3 { animation: spin-cw 1.5s linear infinite; }
                .state-checking .ui-idle { opacity: 1; transform: scale(1); } .state-checking .ui-angry, .state-checking .ui-happy { opacity: 0; transform: scale(0.5); }

                .state-angry .machine-hover { animation: glitch-shake 0.15s infinite; }
                .state-angry .theme-color { stroke: #ef4444; } .state-angry .theme-fill { fill: #ef4444; } .state-angry .aura-glow { fill: #ef4444; opacity: 0.15; }
                .state-angry .scale-1 { transform: scale(0.9); } .state-angry .scale-2 { transform: scale(0.85); } .state-angry .scale-3 { transform: scale(0.8); }
                .state-angry .ring-1 { animation: spin-cw 4s linear infinite; } .state-angry .ring-2 { animation: spin-ccw 3s linear infinite; } .state-angry .ring-3 { animation: spin-cw 2s linear infinite; }
                .state-angry .ui-angry { opacity: 1; transform: scale(1.1); } .state-angry .ui-idle, .state-angry .ui-happy { opacity: 0; transform: scale(0.5); }

                .state-happy .theme-color { stroke: #10b981; } .state-happy .theme-fill { fill: #10b981; } .state-happy .aura-glow { fill: #10b981; opacity: 0.15; }
                .state-happy .scale-1 { transform: scale(1.15); opacity: 0.6; } .state-happy .scale-2 { transform: scale(1.2); opacity: 0.4; } .state-happy .scale-3 { transform: scale(1.25); opacity: 0.2; }
                .state-happy .ring-1 { animation: spin-cw 15s linear infinite; } .state-happy .ring-2 { animation: spin-ccw 12s linear infinite; } .state-happy .ring-3 { animation: spin-cw 10s linear infinite; }
                .state-happy .ui-happy { opacity: 1; transform: scale(1.2); } .state-happy .ui-idle, .state-happy .ui-angry { opacity: 0; transform: scale(0.5); }
            `}</style>
            
            {/* Toast Pop-up Dropdown */}
            <div className={`fixed top-4 right-4 z-[9999] transition-all duration-500 transform ${toast.show ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
                <div className={`px-4 py-3 rounded-lg shadow-lg flex items-center gap-3 border ${toast.type === 'error' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-emerald-50 border-emerald-200 text-emerald-700'}`}>
                    {toast.type === 'error' ? <AlertTriangleIcon /> : <CheckCircleIcon className="w-5 h-5"/>}
                    <span className="font-bold text-sm tracking-wide">{toast.message}</span>
                </div>
            </div>

            <div className="flex flex-col items-center justify-center w-full max-w-sm px-4 z-10">
                <div className="w-[280px] h-[280px] mb-2 relative">
                    <svg viewBox="600 200 720 640" style={{ width: '100%', height: '100%' }} className={`state-${animState}`}>
                        <defs>
                            <filter id="aura-blur" x="-20%" y="-20%" width="140%" height="140%"><feGaussianBlur stdDeviation="30" /></filter>
                        </defs>
                        <g className="machine-hover">
                            <circle cx="960" cy="480" r="220" class="color-sync aura-glow" filter="url(#aura-blur)" />
                            <g class="scale-sync scale-1"><g class="ring-1">
                                <circle cx="960" cy="480" r="240" fill="none" class="color-sync theme-color" strokeWidth="2" strokeDasharray="2 12" opacity="0.4" />
                                <circle cx="960" cy="480" r="240" fill="none" class="color-sync theme-color" strokeWidth="4" strokeDasharray="150 400 50 300" />
                                <circle cx="960" cy="240" r="8" class="color-sync theme-fill" />
                                <circle cx="960" cy="720" r="8" class="color-sync theme-fill" />
                            </g></g>
                            <g class="scale-sync scale-2"><g class="ring-2">
                                <circle cx="960" cy="480" r="180" fill="none" class="color-sync theme-color" strokeWidth="1" opacity="0.3" />
                                <path d="M 830 350 A 180 180 0 0 1 1090 350 M 830 610 A 180 180 0 0 0 1090 610" fill="none" class="color-sync theme-color" strokeWidth="6" strokeLinecap="square" />
                                <path d="M 780 480 L 810 480 M 1110 480 L 1140 480 M 960 300 L 960 330 M 960 630 L 960 660" class="color-sync theme-color" strokeWidth="3" opacity="0.8" />
                            </g></g>
                            <g class="scale-sync scale-3"><g class="ring-3">
                                <circle cx="960" cy="480" r="120" fill="none" class="color-sync theme-color" strokeWidth="15" strokeDasharray="5 15 2 10 30 10" opacity="0.7" />
                            </g></g>
                            <g class="scale-sync scale-3">
                                <circle cx="960" cy="480" r="85" fill="#ffffff" class="color-sync theme-color" strokeWidth="3" />
                                <g class="opacity-sync ui-idle">
                                    <rect x="940" y="475" width="40" height="28" rx="4" fill="none" class="color-sync theme-color" strokeWidth="4" />
                                    <path d="M 948 475 V 460 A 12 12 0 0 1 972 460 V 475" fill="none" class="color-sync theme-color" strokeWidth="4" strokeLinecap="round" />
                                    <circle cx="960" cy="489" r="4" class="color-sync theme-fill" />
                                </g>
                                <g class="opacity-sync ui-angry">
                                    <path d="M 935 455 L 985 505 M 985 455 L 935 505" fill="none" class="color-sync theme-color" strokeWidth="8" strokeLinecap="round" />
                                </g>
                                <g class="opacity-sync ui-happy">
                                    <rect x="940" y="480" width="40" height="28" rx="4" fill="none" class="color-sync theme-color" strokeWidth="4" opacity="0.3" />
                                    <path d="M 948 480 V 460 A 12 12 0 0 1 972 460" fill="none" class="color-sync theme-color" strokeWidth="4" strokeLinecap="round" opacity="0.3" />
                                    <path d="M 935 480 L 955 500 L 990 450" fill="none" class="color-sync theme-color" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" />
                                </g>
                            </g>
                            <g transform="translate(0, 800)">
                                <text x="960" y="0" textAnchor="middle" class="status-text opacity-sync ui-idle color-sync theme-fill">WELCOME</text>
                                <text x="960" y="0" textAnchor="middle" class="status-text opacity-sync ui-angry color-sync theme-fill">FAILED</text>
                                <text x="960" y="0" textAnchor="middle" class="status-text opacity-sync ui-happy color-sync theme-fill">SUCCESS</text>
                            </g>
                        </g>
                    </svg>
                </div>
                ...
                <div className="w-full bg-white p-6 border border-slate-200 shadow-xl flex flex-col gap-4 relative z-10">
                    ...
                    <input type="email" value={loginInput} onChange={e => setLoginInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} className="w-full p-3 bg-white border-2 border-slate-200 text-slate-800 font-bold text-center outline-none transition-all h-12 focus:border-blue-500 focus:shadow-[0_0_10px_rgba(59,130,246,0.2)] disabled:opacity-50" placeholder="MASUKKAN EMAIL ANDA" disabled={animState === 'checking' || animState === 'happy'} />
                    <button onClick={handleLogin} disabled={animState === 'checking' || animState === 'happy'} className="bg-blue-600 text-white border-none p-3 text-base font-bold cursor-pointer shadow-[0_4px_0_rgba(0,0,0,0.1)] transition-all active:translate-y-[2px] active:shadow-[0_2px_0_rgba(0,0,0,0.1)] hover:bg-blue-700 disabled:opacity-50">LOGIN</button>
                </div>
            </div>
        </div>
      );
  ...
  return (
    <>
      ...
      {/* Toast Pop-up Dropdown (Main App) */}
      <div className={`fixed top-4 right-4 z-[9999] transition-all duration-500 transform ${toast.show ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
          <div className={`px-4 py-3 rounded-lg shadow-lg flex items-center gap-3 border ${toast.type === 'error' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-emerald-50 border-emerald-200 text-emerald-700'}`}>
              {toast.type === 'error' ? <AlertTriangleIcon /> : <CheckCircleIcon className="w-5 h-5"/>}
              <span className="font-bold text-sm tracking-wide">{toast.message}</span>
      ...
      <div className="min-h-screen lg:h-screen lg:overflow-hidden bg-slate-100 text-slate-900 flex flex-col">
        ...
        <header className="bg-[#0f172a] border-b border-slate-800 sticky top-0 z-30 shadow-md h-14 flex items-center shrink-0">
          <div className="w-full px-4 sm:px-6 flex justify-between items-center">
            {/* TAUTAN DINONAKTIFKAN SESUAI PERMINTAAN */}
          ...
          <aside className="w-full lg:w-[380px] bg-slate-50 lg:border-r border-slate-200 flex flex-col z-20 shrink-0 lg:h-full lg:overflow-hidden">
            ...
            <div className="flex-1 flex flex-col overflow-y-visible lg:overflow-y-auto overflow-x-hidden custom-scroll">
              <div className="p-4 flex flex-col gap-4">
                ...
                {/* --- PANEL PENGGUNA (MENGGANTIKAN MY PROJECT & SUPPORT) --- */}
                <div className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg shadow-sm">
                    <div className="flex items-center gap-2 overflow-hidden">
                        ...
                        <div className="flex flex-col min-w-0">
                            ...
                            <span className="text-xs font-bold text-slate-700 truncate pr-2">{userEmail}</span>
                        </div>
                    </div>
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm border border-blue-200">
                  <div className="flex items-center justify-between mb-3 pb-1.5 border-b border-blue-100">
                    ...
                    {/* INDIKATOR STATUS ONLINE/OFFLINE TELAH DIHAPUS */}
                  </div>
            ...
            <div className="bg-white border-b border-slate-200 p-3 flex justify-between items-center shrink-0 shadow-sm z-10">
                <div className="flex items-center gap-2 text-sm font-bold text-slate-600">
                    {[50, 100, 150, 200, 250].map(num => (
                        <button key={num} onClick={() => { setItemsPerPage(num); setCurrentPage(1); }} className={`px-2 py-1 rounded border transition ${itemsPerPage === num ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-slate-50 text-slate-500 hover:bg-slate-100 border-slate-200'}`}>
                            {num}
                        </button>
                    ))}
                </div>
                <div className="flex items-center gap-3">
                    <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-1 rounded bg-slate-100 hover:bg-slate-200 disabled:opacity-50 border border-slate-200 transition"><ChevronLeftIcon /></button>
                    <div className="flex items-center gap-2">
                        <input type="number" value={pageInput} onChange={handlePageInput} onKeyDown={handlePageEnter} className="w-12 h-7 text-center border border-slate-300 rounded text-xs font-bold text-slate-700 focus:border-blue-500 outline-none transition-colors" />
                        <span className="text-xs font-bold text-slate-500">/ {totalPages || 1}</span>
                    </div>
                    <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => setCurrentPage(p => p + 1)} className="p-1 rounded bg-slate-100 hover:bg-slate-200 disabled:opacity-50 border border-slate-200 transition"><ChevronRightIcon /></button>
                </div>
            </div>
