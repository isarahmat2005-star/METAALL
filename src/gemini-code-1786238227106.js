...
export default function App() {
  ...
  // Modifikasi String Pemeriksa (FREE META -> METAALL = 77,69,84,65,65,76,76)
  ...
  // URL Proxy Google AI Studio Anda
  ...
  const fetchWithRetry = async (payload, retries = 5, signal) => {
    ...
    // Mengambil prompt teks dan gambar dari format payload yang ada
    ...
    for (let i = 0; i < retries; i++) {
      try {
        // Mengirimkan request ke Server Proxy Google AI Studio (Tanpa API Key di Vercel)
        const res = await fetch(AI_STUDIO_PROXY_URL, {
          method: 'POST',
        ...
        if (!res.ok || !data.success) {
          throw new Error(data.error || 'Gagal memproses di Google AI Studio');
        }

        // Mengembalikan format data agar cocok dengan JSON parser bawaan di App.jsx Anda
        return {
          candidates: [
            {
              content: {
      ...
      <div className="min-h-screen lg:h-screen lg:overflow-hidden bg-slate-100 text-slate-900 flex flex-col">
        ...
        <header className="bg-[#0f172a] border-b border-slate-800 sticky top-0 z-30 shadow-md h-14 flex items-center shrink-0">
          <div className="w-full px-4 sm:px-6 flex justify-between items-center">
            {/* LOGO TITLE TELAH DIGANTI MENJADI METAALL */}
