...
export default function App() {
  ...
  const fetchWithRetry = async (payload, retries = 5, signal) => {
    ...
    // Ekstrak instruksi teks & data gambar dari payload
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (!data.success) {
          throw new Error(data.error || 'Gagal memproses di Google AI Studio Proxy');
        }

        // Format return sesuai yang dibutuhkan JSON.parse pada App.jsx
        return {
          candidates: [
            {
              content: {
